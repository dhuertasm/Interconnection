...
public class ArregloDinamico<T extends Comparable<T>> implements ILista<T> {
		...
		for (int i = 1; i <= this.size(); i++) {
			...
			for (int j = i + 1; j <= this.size(); j++) {
				int factorComparacion = (ascendente ? 1 : -1)
						* criterio.compare(this.getElement(posMayorMenor), this.getElement(j));
	...
	public int partition(ILista<T> lista, Comparator<T> criterio, boolean ascendente, int lo, int hi)
			throws PosException, VacioException {
		int follower, leader;
		follower = leader = lo;
